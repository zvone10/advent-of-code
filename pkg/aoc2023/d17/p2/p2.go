...
func Run2() {
	...
	for len(q) > 0 {
		...
		for _, d := range directions {
			...
			if (d.dr != n.dr || d.dc != n.dc) && n.consecutiveSteps < 3 && !(n.dr == 0 && n.dc == 0) {
				continue
			}
			...
			if !newNodeCostExists || cost+graph[r][c] < costs[newNode] {
	...
	minimum := math.MaxInt
	for n, c := range costs {
		if n.c == len(graph[0])-1 && n.r == len(graph)-1 && n.consecutiveSteps >= 3 {
			fmt.Println(n)
			fmt.Println(c)
